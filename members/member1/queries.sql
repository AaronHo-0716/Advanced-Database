...
--     arrival port.
-- ============================================================
DECLARE @start_date DATE       = '2025-06-01';
DECLARE @end_date   DATE       = '2025-06-30';
DECLARE @dep_code   VARCHAR(10) = 'SIN';
DECLARE @arr_code   VARCHAR(10) = 'SIN';
...
WHERE v.[itinerary_type] = 'Round-trip'
  AND dep.[port_code]   = @dep_code
  AND arr.[port_code]   = @arr_code
  AND CAST(v.[departure_datetime] AS DATE) BETWEEN @start_date AND @end_date
...
--      Expected revenue = adult_fare x SUM(max_occupancy) over
--      all cabins of that category on the ship.
--      ROLLUP produces the per-ship total row.
-- ============================================================
DECLARE @operator_name VARCHAR(100) = 'Royal Caribbean International';
DECLARE @voyage_id     INT          = 1;
...
WHERE op.[operator_name] = @operator_name
  AND v.[voyage_id]      = @voyage_id
...
--      passengers for a specified departure port in a given
--      date range.
--      Interpretation: "booked" means RESERVATION.booking_date
--      sits inside the range. Swap to v.[departure_datetime] if
--      the brief means sailing date instead.
-- ============================================================
DECLARE @dep_code   VARCHAR(10) = 'SIN';
...
JOIN [SHIP] s             ON s.[ship_id]         = v.[ship_id]
JOIN [CRUISE_OPERATOR] op ON op.[operator_id]    = s.[operator_id]
WHERE dep.[port_code] = @dep_code
  AND CAST(r.[booking_date] AS DATE) BETWEEN @start_date AND @end_date
...
-- ============================================================
-- (v) Per age category, total passengers (infants/children/
--     teens/adults/seniors) on a specified voyage operated by a
--     given cruise line. ROLLUP gives detail rows + grand total.
-- ============================================================
DECLARE @voyage_id     INT          = 1;
DECLARE @operator_name VARCHAR(100) = 'Royal Caribbean International';
...
JOIN [SHIP] s               ON s.[ship_id]          = v.[ship_id]
JOIN [CRUISE_OPERATOR] op   ON op.[operator_id]     = s.[operator_id]
WHERE v.[voyage_id]      = @voyage_id
  AND op.[operator_name] = @operator_name
...
--      VOYAGE.arrival_port_id is treated as the FINAL arrival
--      port (ITINERARY_STOP holds only intermediate stops).
-- ============================================================
WITH [OperatorMD] AS (
    SELECT
        op.[operator_id],
...
--       For each operator: total reservations, cancellations,
--       refund payout, retained cancellation fees, and the
--       cancellation rate. Surfaces revenue at risk by brand.
-- ============================================================
WITH [ResByOp] AS (
    SELECT
        op.[operator_id],
